...
% pros: uses existing syntax \texttt{[...]}
% cons: necessity to call use len() to get the length,
%       difficulty of implementing type arguments using expressions (e.g. len([...]T) + 1)
%       inconsistent, something that varies should be an explicit parameter
%       awkwardness of nested structures, using "dummy" arrays as types
