...
Because $\methods_\Delta$ is not defined on \kw{const} (see
\ref{sec:fgg-appx-typing}), there is an additional subtyping rule that specifies
that type parameters which are bound by \kw{const}, are also subtypes of
\kw{const} (rule $\imp_{const-\text{Param}}$). All type parameters are subtypes
of their bounds, but where the bounds are a regular interface (i.e. not
\kw{const}), this can already be derived via the $\imp_I$ rule.
